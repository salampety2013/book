                                            ...
                                            <div class="item  col-lg-4 col-md-6 col-xs-12 text-center no-padding">
                                                <div class="product-miniature js-product-miniature item-one"
                                                    ...
                                                    <div class="thumbnail-container">
                                                        <a href="{{ route('product.details',$advertisment-> slug_ar) }}"
                                                           class="thumbnail product-thumbnail two-image">
                                                            <img class="img-fluid image-cover"
                                                                 src="{{asset('assets/images/products/'.$product->img)   ?? ''}}{{--$advertisment-> images[0] -> img ?? ''--}}"
                                                                 alt=""
                                                                 data-full-size-image-url="{{--$advertisment-> images[0] -> img ?? ''--}} "
                                                            ...
                                                            <div class="group-reviews">
                                                                ...
                                                                <div class="info-stock ml-auto">
                                                                    ...
                                                                    <i class="fa fa-check-square-o"
                                                                       aria-hidden="true"></i>
                                                                    {{$advertisment-> in_stock ? 'in stock' : 'out of stock'}}
                                                                </div>
                                                            </div>

                                                            <div class="product-title" itemprop="name"><a
                                                                    href="{{ route('product.details',$advertisment-> slug_ar) }}">{{$advertisment-> name_ar}}</a></div>

                                                            <div class="product-group-price">
                                                                <div class="product-price-and-shipping">

 @include('front.products.products_price')
                                                                    <?php /*?><span itemprop="price"
                                                    class="price">{{$advertisment-> discount_price ?? $advertisment-> price }}</span>
                                                                    @if($advertisment-> discount_price)
                                                                        <span
                                                                            class="regular-price">{{$advertisment-> price}}</span>
                                                                    @endif<?php */?>
                                                        ...
                                                        <div class="product-buttons d-flex justify-content-center"
                                                            ...
                                                            <form
                                                                ...
                                                                <a class="add-to-cart cart-addition" data-product-id="{{$advertisment-> id}}" data-product-slug="{{$advertisment-> slug}}" href="#"
                                                                   data-button-action="add-to-cart"><i
                                                                        class="novicon-cart"></i><span>Add to cart</span></a>
                                                            </form>

                                                            <a class="addToWishlist  wishlistProd_22" href="#"
                                                               data-product-id="{{$advertisment-> id}}"
                                                            >
...
@section('scripts')
    <script>
        ...
        $(document).on('click', '.check', function (e) {
            ...
            $.ajax({
                type: 'post',
                url: "{{Route('site.advertisment.checkReservations')}}",
                data: {
                    'advertisment_id': $(this).attr('data-advertisment-id'),
                    'start_date' : $(this).attr('data-advertisment-start-date'),
                    'end_date' : $(this).attr('data-advertisment-end-date'),
                },
                success: function (data) {
