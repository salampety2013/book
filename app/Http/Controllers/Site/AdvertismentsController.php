...
class AdvertismentsController extends Controller
{
    ...
    //     with(['options' =>function($q) { $q->select('id','name_ar','name_en','img'); }])->
         ...
         //    $data['advertisment'] = Advertisment:: where('is_active',"1") ->where('slug_'.$lang,$slug)->first();  //improve select only required fields
             $data['advertisment'] = Advertisment:: where('is_active',"1") ->where('slug_ar',$slug)->orwhere('slug_en',$slug)->first();  //improve select only required fields
...
public function checkReservations( Request $request)
{
    ...
    //10-11- 15-  18-19-20-21
    Session::put('start_date',$start_date) ;
    Session::put('end_date',$end_date) ;
    Session::put('advertisment_id',$advertisment_id) ;

//check if that ads that dont have reservation between that date  
            $check = Advertisment::doesntHave('orders', 'or', function ($q) use ($start_date, $end_date) {
        $q->whereBetween('start_date', [$start_date,$end_date])
        ->orWhereBetween('end_date', [$start_date,$end_date]);
    })->where('is_active', "1")->where('id', $advertisment_id)->count();
...
$start_date= date('2023-05-22');
$end_date = date('2023-05-22');
//10-11- 15-  18-19-20-21
...
})->where('is_active', "1")->where('id','29')->get();

/*  return  $query = Advertisment::doesntHave('orders','or',function($q) use($start_date,$end_date){
    $q->whereBetween('start_date', [$start_date,$end_date])
  ->whereBetween('end_date', [$start_date,$end_date]);

})->where('is_active', "1")->get();  // get all advertise that doesntHave reservation in day
...
})->where('is_active', "1")->where('id','29')->count();   ///check for only one day
 */
